...
def main(argv: list[str] | None = None) -> int:
    ...
    for line_number, line in enumerate(args.predictions.read_text(encoding="utf-8").splitlines(), 1):
        ...
        rows.append(row)
    evaluations = [build_exact_match_evaluation(rows, dataset=args.dataset, dataset_sha256=args.dataset_sha256)]
    if args.include_camera:
        evaluations.append(build_camera_accuracy_evaluation(rows, dataset=args.dataset, dataset_sha256=args.dataset_sha256))
